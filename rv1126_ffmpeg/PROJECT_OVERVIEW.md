...
- 输出封装：`0` 表示 FLV，常用于 RTMP；`1` 表示 MPEG-TS，SRT 相关链路目前还没有完全打通。
...
当前工程优先测试 RTMP/FLV。主码流和子码流都可以先用 `0` 进行验证。

```bash
./rv1126_ffmpeg_main 0 rtmp://192.168.1.22:1935/live/01 0 rtmp://192.168.1.22:1935/live/02
```
...
- 第 3 个参数：子码流协议类型。
- 第 4 个参数：子码流推流地址。
- `0` 表示 FLV，通常用于 RTMP 推流。
- `1` 表示 MPEG-TS，目前仓库里这条链路还没有完全实现，先不要当成稳定可用的 SRT 能力。

如果先做联调，建议优先验证主码流 RTMP 是否能正常出流，再测试双码流同时推送。

## 播放地址说明

在播放端命令中，地址里的 IP 指的是“接收/播放端”的地址，而不是 RV1126 板子的地址。

例如：

```bash
./ffplay.exe -x 400 -y 400 rtmp://192.168.1.66:1935/live/01
```

这里的 `192.168.1.66` 表示 Windows 机器的 IP，说明 ffplay 会去连接这台 Windows 机器上的 RTMP 服务；这台机器必须已经在监听 1935 端口，并且能接收推流。

因此：

- 如果你是在 Windows 本机播放，IP 需要填 Windows 本机的 IP。
- 如果你是在另一台电脑播放，IP 需要填那台播放/接收电脑的 IP。
- 如果只是把推流地址写成板子 IP，而没有在那台机器上部署 RTMP 服务，播放就不会成功。
